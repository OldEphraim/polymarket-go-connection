-- +goose Up
-- Convert market_trades to hourly partitioning
...
ALTER SEQUENCE market_trades_id_seq OWNED BY market_trades.id;
ALTER TABLE market_trades ALTER COLUMN id SET DEFAULT nextval('market_trades_id_seq'::regclass);

-- 4) Helper function for hourly partitions
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION create_market_trades_partition_hourly(p_hour timestamptz)
RETURNS void
LANGUAGE plpgsql
AS $func$
DECLARE
...
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION ensure_trades_partitions_hourly(p_hours_back int DEFAULT 3,
...
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION drop_archived_market_trades_partitions_hourly(p_keep_hours int)
RETURNS int
LANGUAGE plpgsql
AS $func$
DECLARE
...
SELECT ensure_trades_partitions_hourly(3, 3);

-- 8) Create view for old data
CREATE OR REPLACE VIEW market_trades_old_view AS
  SELECT * FROM market_trades_old;

-- +goose Down
DROP VIEW IF EXISTS market_trades_old_view;
DROP FUNCTION IF EXISTS drop_archived_market_trades_partitions_hourly(int);
DROP FUNCTION IF EXISTS ensure_trades_partitions_hourly(int, int);
DROP FUNCTION IF EXISTS create_market_trades_partition_hourly(timestamptz);
...
DECLARE
  drop_children text;
BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'market_trades' AND relkind = 'p') THEN
    ...
    EXECUTE 'DROP TABLE IF EXISTS market_trades';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'market_trades_old') THEN
    EXECUTE 'ALTER TABLE market_trades_old RENAME TO market_trades';
  END IF;
END$$;
